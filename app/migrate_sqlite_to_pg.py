...
print("🔧 Creating tables with correct schema (manual for problematic ones)...")
with sync_engine.connect() as conn:
    # Create listings with ALL columns (including the missing ones)
    conn.execute(text("""
    ...
    # Create services with is_active
    conn.execute(text("""
    ...
    conn.commit()
print("✅ Problematic tables created with correct schema.")

# ─── Create the rest of the tables using SQLAlchemy (skip existing) ──────
print("🔧 Creating remaining tables with SQLAlchemy...")
Base.metadata.create_all(bind=sync_engine)
print("✅ All tables created.")

# ─── Copy data using psycopg2 ──────────────────────────────────────────────
SQLITE_PATH = "seai.db"
